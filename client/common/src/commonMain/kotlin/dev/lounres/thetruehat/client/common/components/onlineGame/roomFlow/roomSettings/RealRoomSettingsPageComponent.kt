...
public class RealRoomSettingsPageComponent(
    ...
    override val availableDictionaries: Value<List<UserGameState.ServerDictionary>>,
    ...
    override val dictionary: Value<UserGameState.ServerDictionary> = settingsUpdate.map { update ->
        when(val wordsSource = update.wordsSource ?: settings.wordsSource) {
            is Settings.WordsSource.ServerDictionary -> availableDictionaries.value.first { it.id == wordsSource.id }
        }
    }
    override val onDictionaryChange: (Settings.WordsSource) -> Unit = { input -> settingsUpdate.update { it.copy(wordsSource = input) } }
